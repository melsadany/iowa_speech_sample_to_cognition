...
library(text);textrpp_initialize()

## to save time, I save embeddings for unique words I get.
##    I load the saved embeddings first, and check if there're any new words
##    if there is, I extract embeddings for new ones only and save
##    reasoning for this is that it takes a very long time to extract embeddings

## load old embeddings
load("data/derivatives/tmp-use/word-embedding-from-text-package.rda")
new.words <- unique(psvc.tx$response)[!(unique(psvc.tx$response) %in% emb.all$text)]

if (length(new.words) > 0) {
  emb.0 <- textEmbed(new.words,tokenizer_parallelism = T)
  emb.1 <- emb.0$texts$texts
  emb.2 <- cbind(text = new.words, emb.1)
  emb.2.o <- emb.all
  emb.all <- rbind(emb.2.o, emb.2)
  save(emb.all, file = "data/derivatives/tmp-use/word-embedding-from-text-package.rda")
}
...
# keep new pairs
prev.sim <- read_rds("data/derivatives/tmp-use/pairs-similarity-all.rds")
...
if (nrow(pairs.to.run)>0) {
  ...
  write_rds(pairs.sim, "data/derivatives/tmp-use/pairs-similarity-all.rds", compress = "gz")
  rm(pairs.sim.n)
}
rm(pairs.to.run);rm(pairs.to.run.n);rm(prev.sim)
pairs.sim <- read_rds("data/derivatives/tmp-use/pairs-similarity-all.rds")
...
write_rds(all.pairs.sim.per.prompt, "data/derivatives/tmp-use/PSVC-pairs-similarity-per-prompt.rds", compress = "gz")
all.pairs.sim.per.prompt <- read_rds("data/derivatives/tmp-use/PSVC-pairs-similarity-per-prompt.rds")
...
ggsave2("figs/PSVC/distributions/cos-similarity-histogram.png",width = 6, height = 4)
...
euc %>% mutate(path_euclidean_distance_normalized = path_euclidean_distance/count) %>%
  select(-count) %>%
  write_rds("data/derivatives/tmp-use/PSVC-path-euclidean-distance-per-prompt.rds")
...
div %>%
  write_rds("data/derivatives/tmp-use/PSVC-divergence-prompt-starting-per-prompt.rds")
...
umap.0 %>% write_rds("data/derivatives/tmp-use/UMAP-on-word-embedding-from-text-package.rds", compress = "gz")
...
registerDoMC(6)
umap.0 <- read_rds("data/derivatives/tmp-use/UMAP-on-word-embedding-from-text-package.rds")
...
voc.vol %>%
  write_rds("data/derivatives/tmp-use/PSVC-vocabulary-volume-per-prompt.rds")
...
library(archetypes)
stepArchetypes(emb.all[,-1] %>% as.matrix(), k = 10, nrep = 4, verbose = T) %>%
  write_rds("data/derivatives/tmp-use/archetypes-on-word-embedding-from-text-package.rds")
arch <- bestModel(read_rds("data/derivatives/tmp-use/archetypes-on-word-embedding-from-text-package.rds"))
arch.summ <- archetypes_summ(obj = arch, k = 10, points_labels = emb.all$text)
...
write_rds(arch.simplex, "figs/PSVC/obj/semantic-archetypes.simplex.rds", compress = "gz")
...
arch.metrics %>% write_rds("data/derivatives/tmp-use/PSVC-communities-meta-per-prompt.rds")
...
arch.area %>% write_rds("data/derivatives/tmp-use/PSVC-archetypal-area-per-prompt.rds")
...
arch.richness %>% write_rds("data/derivatives/tmp-use/PSVC-archetypal-richness-per-prompt.rds")
...
participant.norms <- psvc.tx %>% distinct(te_id, prompt, response) %>%
  ...
  rbind(psvc.tx %>% distinct(te_id, response) %>%
          ...
          summarise(l1_norm = mean(l1_norm),l2_norm = mean(l2_norm),
                    ...
                    l1_norm_origin_avg_set = mean(l1_norm_origin_avg_set),l2_norm_origin_avg_set = mean(l2_norm_origin_avg_set)) %>%
          mutate(prompt="all"))
participant.norms %>% write_rds("data/derivatives/tmp-use/PSVC-norms-per-prompt.rds")
...
prompt.sim.traj <- pairs.to.prompt %>% 
  ...
  summarise(prompt_sim_trajectory = tryCatch({coef(lm(cosine_similarity ~ start))[2]}, 
            ...
            n_obs = n(),.groups = 'drop')
prompt.sim.traj %>% write_rds("data/derivatives/tmp-use/PSVC-prompt-similarity-trajectory-per-prompt.rds",
                              compress = "gz")
