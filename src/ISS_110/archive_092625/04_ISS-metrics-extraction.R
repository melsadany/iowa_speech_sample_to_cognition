...
iss.meta.me <- readxl::read_xlsx("../shared_data/data/RPOE_meta.xlsx", sheet = "ISS-metadata") %>%
  filter(task_v=="1.10B")
m1m2 <- read_rds("../shared_data/data/m1m2.rds")
m1m2.s <- read_rds("../shared_data/data/m1m2-sex-corrected.rds")
################################################################################
################################################################################
# read the cleaned tx
...
#   1st batch
srt.01 <- readxl::read_xlsx("../shared_data/data/RPOE_meta.xlsx", sheet = "ISS_data_evaluation",
...
mem.count <- readxl::read_xlsx("../shared_data/data/RPOE_meta.xlsx", sheet = "ISS_transcription_110B") %>%
...
iss.start <- readxl::read_xlsx("../shared_data/data/RPOE_meta.xlsx", sheet = "ISS_det") %>%
...
wa.task.start.time <- clean.tx %>% filter(pred_task=="wmem") %>% 
  ...
  select(te_id, diff)
# wa.task.start.time <- clean.tx %>% filter(pred_task=="word_association") %>%
#   group_by(te_id) %>% slice_tail(n = 1) %>% select(te_id, tx_task_start = end)
wa.onset <- wa.tx %>%
  left_join(iss.start) %>% mutate(start = start-diff1) %>% 
  # left_join(wa.task.start.time) %>% mutate(start = start + diff) %>%
  left_join(ann[20:41,] %>% mutate(ann_task_start = seconds-34)%>% select(pred_task=prompt,ann_task_start)) %>%
  # inner_join(iss.meta.me %>% mutate(ann_task_start = start_in_sec-34) %>%
  #              select(pred_task, ann_task_start) %>% drop_na(pred_task)) %>% 
...
rpoe.all <- read_rds("../shared_data/data/all-summarized-data.rds")
