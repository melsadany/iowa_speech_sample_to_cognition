...
################################################################################
################################################################################
all.tx.raw.o <- readxl::read_xlsx("data/RPOE_meta.xlsx", sheet = "ISS_transcription_110B")
...
################################################################################
################################################################################
raw.tx <- readxl::read_xlsx("data/RPOE_meta.xlsx", sheet = "ISS_transcription_110B") %>%
  rename(prompt = pred_task, response=word,text_revised=word_revised) %>%
  mutate(response = tolower(response), prompt = tolower(prompt),text_revised=tolower(text_revised),
         prompt = ifelse(prompt=="what was the first word you said in that run?","wmem",prompt))

## need to divide each task independently
wat.tx <- raw.tx %>% filter(prompt %in% c(tolower(ann$prompt)[20:41],"wmem"))
srt <- raw.tx %>% filter(grepl("response",prompt))
################################################################################
################################################################################
################################################################################
## count comments
...
write_rds(comments, "data/derivatives/tmp-use/ISS/comments-count.rds", compress = "gz")

## drop ums, comments, and false tx words
clean.tx.0 <- wat.tx %>% 
  mutate(response = ifelse(is.na(text_revised), response, text_revised)) %>%
  filter(is.na(text_revised)|text_revised!="f", 
         !response %in% c("uh", "um", "umm", "oh", "eh", "hmm", "hmmm", "a", "mmm","W?", "yeah", "okay"),
         is.na(comment)) %>%
  mutate(duration = end-start) %>%
  select(-c(text_revised,comment,initials))

## count repeated prompts
rep.prompt <- clean.tx.0 %>% filter(prompt!="wmem") %>%
  group_by(te_id, prompt) %>%
  summarise(rep_prompt = sum(prompt==response,na.rm=T))
write_rds(rep.prompt, "data/derivatives/tmp-use/ISS/repeated-prompt-count.rds", compress = "gz")

## count repeated words per prompt
rep.words.0 <- clean.tx.0 %>% filter(prompt!="wmem") %>%
  group_by(te_id, prompt) %>%
  summarise(rep_words_per_prompt = n()-length(unique(response)))
write_rds(rep.words.0, "data/derivatives/tmp-use/ISS/repeated-words-per-prompt-count.rds", compress = "gz")

## count repeated words in the entire task
rep.words.1 <- clean.tx.0 %>% filter(prompt!="wmem") %>%
  group_by(te_id) %>%
  summarise(rep_words = n()-length(unique(response)))
write_rds(rep.words.1, "data/derivatives/tmp-use/ISS/repeated-words-in-the-entire-task-count.rds", compress = "gz")

## wmem responses
wmem.resp <- clean.tx.0 %>% 
  filter(!response %in% unlist(str_split("what was the first word you said in that run"," "))) %>%
  group_by(te_id,prompt) %>% ungroup() %>% mutate(prev_task = lag(prompt)) %>%
  filter(prompt=="wmem") %>% 
  select(te_id, wmem_response=response,prompt=prev_task)

wmem.prompt <- c("begins with the letter s", "begins with the letter f", "begins with the letter a", "begins with the letter p")
wmem.scores <- clean.tx.0 %>% filter(prompt %in% wmem.prompt) %>% select(te_id, prompt, response) %>%
  full_join(wmem.resp) %>% group_by(te_id,prompt) %>% 
  mutate(order = c(1:n()),this_one = wmem_response==response, score = case_when(this_one ~ 1/order)) %>%
  filter(!is.na(score)) %>% slice_max(order_by = score,n=1) %>%
  pivot_wider(names_from = prompt, values_from = score,values_fill = 0,id_cols = te_id)
wmem.scores %>% write_rds("data/derivatives/tmp-use/ISS/wmem-scores.rds", compress = "gz")


## clean tx again by dropping repeated words per prompt and repeated prompts
clean.tx.1 <- clean.tx.0 %>% filter(prompt != "wmem") %>%
  filter(prompt!=response) %>% select(-wmem_correct) %>%
  distinct(te_id, prompt, response, .keep_all = T)
write_rds(clean.tx.1, "data/derivatives/ISS-clean-transcription.rds", compress = "gz")
################################################################################
################################################################################
